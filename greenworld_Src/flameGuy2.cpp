...
CFlameGuy2Object::CFlameGuy2Object(char* name, D3DXVECTOR3 location, float xspeed, float yspeed):
CIntelligentObject(FLAMEGUY2_OBJECT, name, location, xspeed, yspeed){ //constructor
  mHeight = location.y;
  mWidth = location.x;
  m_fDistance = m_fHorizontalDistance = m_fVerticalDistance = 0.0f;
  m_eState = CRUISINGS4_STATE;
...
void CFlameGuy2Object::move(){ //move object
...
void CFlameGuy2Object::ai(){ //main AI function
  //do the following periodically
  if(g_cTimer.elapsed(m_nLastAiTime,m_nAiDelayTime)) 
    switch(m_eState){ //behavior depends on state
      case CRUISINGS4_STATE: CruisingAi(); break;
      case AVOIDINGS4_STATE: AvoidingAi(); break;
	  case CHASINGS4_STATE: ChasingAi(); break;
	  case WAITS4_STATE: WaitingAi(); break;
      default: break;
    }
}
...
void CFlameGuy2Object::CruisingAi(){ //just cruising along
...
void CFlameGuy2Object::AvoidingAi(){ //avoiding plane
  ...
  //    m_fVerticalDistance>FALLBACK_DISTANCE)) //higher or lower
    SetState(CRUISINGS4_STATE); //then back to cruising

}

void CFlameGuy2Object::ChasingAi()
{
	...
	if(CLOSE_DISTANCE > m_fDistance)
		SetState(CRUISINGS4_STATE);
}
}
void CFlameGuy2Object::WaitingAi(){ //avoiding plane
	/*
  //height variation
  if(g_cTimer.elapsed(m_nHeightTime, m_nHeightDelayTime)){
    m_nDesiredHeight = g_cRandom.number(100,450);
